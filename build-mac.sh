...
APP_NAME="ClassicBitcoinDesktopWallet"
APP_DISPLAY_NAME="Classic Bitcoin Desktop Wallet"
VERSION="1.1.2"
...
if [ ! -e ./cbtcd ]
then
	echo "please provide cbtcd in the root directory"
else
	echo "found cbtcd - OK"
fi
...
if [ ! -e ./cbtc-cli ]
then
	echo "please provide cbtc-cli in the root directory"
else
	echo "found cbtc-cli - OK"
fi
echo ""
echo "******************"
echo "|| building JAR ||"
...
echo ""
#package jar to app
jar2app build/libs/"${APP_NAME}"-*.jar  -i ./src/main/resources/images/cbtc.icns -b org.cbitcoin.wallets.fullnode -v "${VERSION}" -s "${VERSION}"
...
cp ./cbtcd ./cbtcd-dylib
cp ./cbtc-cli ./cbtc-cli-dylib

#ensure permissions allow for execution
chmod +x "./cbtcd-dylib"
chmod +x "./cbtc-cli-dylib"
...
dylibbundler -od -b -x "./cbtcd-dylib" \
                    -x "./cbtc-cli-dylib" \
                    -d "./libs" \
                    -p @executable_path/libs

#add cbtcd, cbtc-cli, and libs into the required Contents folder of the App
cp ./cbtcd-dylib "./${APP_DISPLAY_NAME}.app/Contents/cbtcd"
cp ./cbtc-cli-dylib "./${APP_DISPLAY_NAME}.app/Contents/cbtc-cli"
