package org.cbitcoin.wallets.fullnode.daemon;
...
public class CBTCInstallationObserver {
    ...
    public CBTCInstallationObserver(String installDir)
            throws IOException {
        ...
        if (!dir.exists() || dir.isFile()) {
            throw new InstallationDetectionException(
                    "The Classic Bitcoin installation directory " + installDir + " does not exist or is not " +
        ...
        Log.info("Using Classic Bitcoin daemon and rpc tools: " +
        ...
        if ((zcashd == null) || (zcashcli == null) || (!zcashd.exists()) || (!zcashcli.exists())) {
            throw new InstallationDetectionException(
                    "The Classic Bitcoin Full-Node Desktop Wallet installation directory " + installDir + " needs\nto contain " +
                            "the command line utilities cbtcd and cbtc-cli. At least one of them is missing! \n" +
                            "Please place files ClassicBitcoinDesktopWallet.jar, " + OSUtil.getZCashCli() + ", " +
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (i == 0) {
                    if (token.equals(daemonName + ".exe") || token.equals(daemonName)) {
                        info.status = DAEMON_STATUS.RUNNING;
                        foundZCash = true;
                        //System.out.println("cbtc process data is: " + line);
                    }
