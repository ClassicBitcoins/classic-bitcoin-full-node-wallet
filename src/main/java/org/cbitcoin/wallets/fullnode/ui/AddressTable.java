...
import org.cbitcoin.wallets.fullnode.daemon.CBTCClientCaller.ShieldCoinbaseResponse;
    ...
    shieldAllCoinbaseFundsMenuItem.addActionListener(e -> {
      if ((lastRow >= 0) && (lastColumn >= 0))
      {
        try
        {
          ...
          JOptionPane.showMessageDialog(
              AddressTable.this.getRootPane().getParent(),
              "Coinbase funds in the amount of " + shieldCoinbaseResponse.shieldedValue + " CBTC from " + shieldCoinbaseResponse.shieldedUTXOs + " UTXO" + (shieldCoinbaseResponse.shieldedUTXOs == 1 ? "" : "s") + " were shielded to the following z-address:\n" + address + "\nPlease Refresh to update balances.\n",
