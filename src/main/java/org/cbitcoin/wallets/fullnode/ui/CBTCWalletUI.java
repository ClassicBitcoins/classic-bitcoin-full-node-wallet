...
import org.cbitcoin.wallets.fullnode.daemon.CBTCInstallationObserver.DAEMON_STATUS;
import org.cbitcoin.wallets.fullnode.daemon.CBTCInstallationObserver.DaemonInfo;
import org.cbitcoin.wallets.fullnode.daemon.CBTCInstallationObserver.InstallationDetectionException;
import org.cbitcoin.wallets.fullnode.messaging.MessagingPanel;
 ...
 */
public class CBTCWalletUI extends JFrame {
    ...
    private static final String IMG_CBTC_ICON = "images/cbtc-200.png";
    ...
    public CBTCWalletUI(StartupProgressDialog progressDialog, String title)
        ...
        this.setIconImage(new ImageIcon(cl.getResource(IMG_CBTC_ICON)).getImage());
        Container contentPane = this.getContentPane();

        errorReporter = new StatusUpdateErrorReporter(this);
        installationObserver = new CBTCInstallationObserver(OSUtil.getProgramDirectory());
        clientCaller = new CBTCClientCaller(OSUtil.getProgramDirectory());
        ...
        menuItemExit.addActionListener(
            e -> CBTCWalletUI.this.exitProgram()
        );

        menuItemAbout.addActionListener(
            e -> {
                try {
                    AboutDialog ad = new AboutDialog(CBTCWalletUI.this);
                    ad.setVisible(true);
                } catch (UnsupportedEncodingException uee) {
                    Log.error("Unexpected error: ", uee);
                    CBTCWalletUI.this.errorReporter.reportError(uee);
                }
            }
        );
        menuItemShowPrivateKey.addActionListener(
            e -> CBTCWalletUI.this.walletOps.showPrivateKey()
        );

        menuItemImportOnePrivateKey.addActionListener(
            e -> CBTCWalletUI.this.walletOps.importSinglePrivateKey()
        );

        menuItemOwnIdentity.addActionListener(
            e -> CBTCWalletUI.this.messagingPanel.openOwnIdentityDialog()
        );

        menuItemExportOwnIdentity.addActionListener(
            e -> CBTCWalletUI.this.messagingPanel.exportOwnIdentity()
        );

        menuItemImportContactIdentity.addActionListener(
            e -> CBTCWalletUI.this.messagingPanel.importContactIdentity()
        );

        menuItemRemoveContactIdentity.addActionListener(
            e -> CBTCWalletUI.this.messagingPanel.removeSelectedContact()
        );

        menuItemMessagingOptions.addActionListener(
            e -> CBTCWalletUI.this.messagingPanel.openOptionsDialog()
        );
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            JOptionPane.showMessageDialog(
                CBTCWalletUI.this.getRootPane().getParent(),
        ...
        tabs.addChangeListener(
            e -> {
                ...
                if (tabs.getSelectedIndex() == 4) {
                    CBTCWalletUI.this.messagingPanel.tabSelected();
                }
            }
        );
    ...
    public void exitProgram() {
        ...
        CBTCWalletUI.this.setVisible(false);
        CBTCWalletUI.this.dispose();
    ...
    public static void main(String argv[])
        ...
        try {
            ...
            Log.info("Classic Bitcoin Full-Node Desktop Wallet (GUI, made in Java & Swing)");
            ...
            CBTCInstallationObserver initialInstallationObserver =
                new CBTCInstallationObserver(OSUtil.getProgramDirectory());
            DaemonInfo zcashdInfo = initialInstallationObserver.getDaemonInfo();
            initialInstallationObserver = null;

            CBTCClientCaller initialClientCaller = new CBTCClientCaller(OSUtil.getProgramDirectory());
            ...
            if ((zcashdInfo.status != DAEMON_STATUS.RUNNING) || (daemonStartInProgress)) {
                Log.info(
                    "cbtcd is not running at the moment or has not started/synchronized 100% - showing splash...");
            ...
            CBTCWalletUI ui = new CBTCWalletUI(startupBar, title);
            ui.setVisible(true);
            ...
            if ((wce.getMessage().indexOf("{\"code\":-28,\"message\"") != -1) ||
                ...
                JOptionPane.showMessageDialog(
                    null,
                    "There was a problem communicating with the Classic Bitcoin daemon/wallet. \n" +
                        "Please ensure that the Classic Bitcoin server cbtcd is started (e.g. via \n" +
                        "command  \"cbtcd --daemon\"). Error Message: \n" +
                        wce.getMessage() +
                        "See the console/logfile output for more detailed error information!",
                    "Daemon Error",
                    JOptionPane.ERROR_MESSAGE);
    ...
    public static void possiblyCreateZENConfigFile()
        ...
        File zenConfigFile = new File(dir, "cbitcoin.conf");

        if (!zenConfigFile.exists()) {

            Log.info("cbitcoin.conf (" + zenConfigFile.getCanonicalPath() +
    ...
    private static List<String> getDefaultConfig() {
        ...
        InputStream is = CBTCWalletUI.class.getResourceAsStream("/config/config.txt");
