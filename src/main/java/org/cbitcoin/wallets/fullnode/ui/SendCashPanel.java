...
import org.cbitcoin.wallets.fullnode.util.BackupTracker;
...
public class SendCashPanel
  ...
  private static final String LOCAL_MSG_SEND_CBTC_FROM = Util.local("LOCAL_MSG_SEND_CBTC_FROM");
  private static final String LOCAL_MSG_SEND_CBTC_ONLY_CONFIRMED = Util.local("LOCAL_MSG_SEND_CBTC_ONLY_CONFIRMED");
  ...
  public SendCashPanel(CBTCClientCaller clientCaller,
    ...
    tempPanel.add(new JLabel(LOCAL_MSG_SEND_CBTC_FROM + ":       "));
    tempPanel.add(new JLabel(
        "<html><span style=\"font-size:0.8em;\">" +
            LOCAL_MSG_SEND_CBTC_ONLY_CONFIRMED +
            "</span>  "));
  ...
  private void sendCash()
    ...
    // https://github.com/ClassicBitcoinS/trezor-common/blob/08fe85ad07bbbdc25cc83ffae8be7aff89245594/coins.json#L575
    // B Addresses are 35 chars (rz, sx)
    // Z Addresses are 95 chars (zk)
    // base58check encoded

    // Prevent accidental sending to non-CBTC addresses (as seems to be supported by daemon)
    if (!installationObserver.isOnTestNet()) {
      if (!(destinationAddress.startsWith("zk") ||
          destinationAddress.startsWith("rz") ||
          destinationAddress.startsWith("sx"))) {
        Object[] options = {"OK"};
  ...
  private void reportCompleteOperationToTheUser(String amount, String sourceAddress, String destinationAddress)
    ...
    if (clientCaller.isCompletedOperationSuccessful(operationStatusID)) {
      ...
      if (option == 1) {
        ...
        String urlPrefix = "http://classicbitcoin.info/explorer/tx/";
        if (installationObserver.isOnTestNet()) {
          urlPrefix = "https://bitclassic.info/explorerg/tx/";
        }
